...
剧本 →(确认)→ 每组画面4张候选 → CODEX 评选自动确认1张 → 分镜 → 首尾帧
→(确认)→ 视频 → 配音 → 剪辑 → 质检 → 成品包。
...
1. **剧本确认**:剧本写完先停(此刻还没画图、不花出图额度),
   ...
   满意点「✅ 剧本OK,开始画图」;
2. **图片资产自动定版**:确认剧本后立刻开画——每个正式角色、每件核心
   道具、每个场景各出 **4 张候选**,再由 **CODEX 评选**按剧本要求打分,
   **自动确认最符合的一张**,不用等人挑图。定版页会写明每组是机器自动
   确认还是人工确认、把握多少、为什么选它;不满意随时点「🤖 图片资产
   定版 · 改选」换一张或「↻ 不满意,换4张」重画,改完自动重新确认。
   只有 CODEX 判定四张都不合格或把握不足时,才会停下来等人工四选一;
3. **开拍确认**:分镜/首尾帧画完再停,画布里逐张检查
   (可附意见重画/上传替换),点「✅ 确认,开始生产」→ 自动完成
   视频(自带配音)/剪辑/质检 → 「⬇ 下载成品包」。

分镜画面默认**每镜只画 1 张**(不给几十个镜头都烧 4 张)。哪一镜不满意，
就在该镜的「要更多选择?」里选 1–4 张现要备选，CODEX 会按本镜要求给推荐，
你挑一张点「用这张换掉当前图」即可，同场首尾帧和旧视频会自动同步作废。
