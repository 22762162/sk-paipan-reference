...
技术硬门（模型真实能力、文件完整性、VIP 720P、即梦内置配音与对口型、
身份/人数/物理等）独立于创作规则，任何层级都不能关闭、降级或覆盖。
...
同一个稳定 `key` 冲突时，只保留最高层的最终值，禁止把互斥文字继续拼入
提示词；不同 `key` 继续叠加。高层可以用 `suppressions` 显式停用低层
创作默认，但不能停用技术硬门。每次裁决都保存来源、覆盖记录和指纹。

穿越、梦境、游戏世界、戏中戏等多时空作品必须为每个镜头登记
`story_phase`、`era_context` 和 `active_realm_id`。现代物件能否出现，
以本镜相位和本镜允许清单为准，不能用整部剧的题材标签推断；相位规则不得
泄漏到其他镜头、其他集或下一部剧。

本节规定“规则属于哪里”；下文原有优先级继续规定同一作用域内人物定版、
镜头合同、Provider 能力和系统回退之间的内部裁决，两者共同生效。
