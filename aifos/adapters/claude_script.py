...
- 另建 `core_props`，只收录会跨镜复用、影响身份识别、承载关键剧情或出错会污染
  ...
  并固定 `candidate_count:4` 供 AI 自动四选一（人工仅可选覆盖）。一次性普通小物只写进场次道具台账，
...
- 运镜要有节奏，成片不是幻灯片：同一场不得连续三镜「固定」，每场至少
  ...
  一镜只用一种，不得整套堆叠；希区柯克变焦属实验级，仅悬疑类题材
  语法明确要求时限量使用；
...
DIRECTOR_PRINCIPLES = (
    ...
    "遵守轴线,两人左右关系不得翻转",
    "画面要动起来:连续多镜「固定」就是幻灯片,每场至少一处有效镜头运动;"
    "情绪极点按题材动势语法用表现性运镜(甩镜/穿越/俯冲/螺旋环绕/升格),"
    "一镜只用一种;希区柯克变焦是实验级,仅题材语法明确要求时限量使用,"
    "预期不稳回退为推",
...
- 运镜是视频专属维度,与静态画面的可拍性无关:除非熔断原因明确指向
  运镜本身,否则禁止删除运镜或改为「固定」;
