...
- 逐场分析空间功能、入口出口、前中后景、材质道具、时段天气、主光方向、
  环境声和连续性锚点;
- scenes 的 environment/layout/materials_and_props/lighting/prompt_prefix
  只描述无人环境:不得出现任何人物名、人称、尸体、人体局部、剪影或人物
  动作调度;本场人物走位与事件只写进 story_function;血迹、凶器等一次性
  事件痕迹属于镜头层,不得写入场景字段——场景概念图是跨集复用的空镜母资产;
