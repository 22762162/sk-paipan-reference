...
async function pollJob(jobId, onDone, onProgress) {
  ...
  const tick = async () => {
    try {
      ...
      // queued 是"还没轮到",不是终态;当成终态会把排队误报成失败。
      ...
      try {
        ...
        pollJob(reply.job_id, (job) => {
          ...
          if (job.status === "queued") {
            btn.textContent = `排队中(${job.queue_position || 1})…`;
  ...
  root.querySelectorAll(".shot-inline-revision").forEach((box) => {
    ...
    go.onclick = async (event) => {
      ...
      try {
        ...
        if (reply.status === "queued") {
          const ahead = reply.queue_position || 1;
          go.textContent = `排队中(前面还有${ahead}张)…`;
          showToast(`已排入重画队列,前面还有${ahead}张;`
            + "不用等,可以继续给其他图片写意见并提交", "ok");
        }
        pollJob(reply.job_id, (job) => {
          if (job.status === "done") {
          ...
          // 队列前进时同步按钮文案,避免一直停在“排队中”看不出进度
          if (job.status === "queued") {
            const ahead = job.queue_position || 1;
            go.textContent = `排队中(前面还有${ahead}张)…`;
          } else if (job.status === "running") {
            go.textContent = "正在重画并同步…";
          }
...
async function ensureBatchRevisionCheckpoint(episodeId) {
  const current = await api(`/api/episode/${episodeId}`);
  // 只有整集生产才需要先暂停。别的单张图正在重画不该触发暂停——
  // 那会把人家正在跑的重画直接打断,而且这一张也没必要等。
  if (!current.production_progress?.overall?.production_running) return current;
  try {
    await api("/api/stop", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ episode_id: episodeId }),
    ...
    box.querySelector(".plan-edit-go").onclick = async () => {
      ...
      try {
        ...
        pollJob(reply.job_id, (job) => {
          ...
          if (job.status === "queued") {
            btn.textContent = `排队中(前面还有${job.queue_position || 1}张)…`;
          } else if (job.status === "running") {
            btn.textContent = "已提交,重画中…";
          }
        });
...
function updateLiveStrip(data) {
  ...
  const stageText = progress.active
    ...
    : "当前没有生产任务";
  const elapsedText = `本步已进行 ${mm}:${ss}${counts ? " · " + counts : ""}${slow}`;
  let dot = el.querySelector(".live-dot");
  let stageEl = el.querySelector(".live-stage");
  let elapsedEl = el.querySelector(".live-elapsed");
  if (!dot || !stageEl || !elapsedEl || !el.querySelector("#live-last")) {
    // 只在首次挂载时建立 DOM；轮询不得反复清空“最新日志”并改变状态条高度。
    el.innerHTML = `
      <span class="live-dot${stage === "cancelling" ? " stopping" : ""}"></span>
      <b class="live-stage">${esc(stageText)}</b>
      <span class="live-elapsed dim">${esc(elapsedText)}</span>
      <span class="live-last dim" id="live-last"></span>`;
    dot = el.querySelector(".live-dot");
    stageEl = el.querySelector(".live-stage");
    elapsedEl = el.querySelector(".live-elapsed");
  } else {
    dot.classList.toggle("stopping", stage === "cancelling");
  ...
  liveTicker = setInterval(async () => {
    ...
    if (secs % 3 === 0) {
      ...
      if (logEl) api("/api/logs?limit=14").then((rows) => {
        ...
        const signature = JSON.stringify(ordered.map((r) => [
          r.id, r.created_at, r.level, r.source, r.message,
        ]));
        if (logEl.dataset.signature === signature) return;
        const stickToBottom = !logEl.dataset.signature
          || logEl.scrollHeight - logEl.scrollTop - logEl.clientHeight < 24;
        const previousScrollTop = logEl.scrollTop;
        logEl.innerHTML = ordered.length ? ordered.map((r) =>
          `<div class="lv-${esc(r.level)}">[${esc(r.level)}] ${esc(r.source)}: ${esc(r.message)}</div>`).join("")
          : `<div class="dim">暂无产线日志</div>`;
        logEl.dataset.signature = signature;
        logEl.scrollTop = stickToBottom
          ? logEl.scrollHeight
          : Math.max(0, Math.min(
            previousScrollTop, logEl.scrollHeight - logEl.clientHeight));
      }).catch(() => {});
    }
    const elapsed = strip.querySelector(".live-elapsed");
    if (elapsed) {
      const mm = Math.floor(secs / 60), ss = String(secs % 60).padStart(2, "0");
...
function stableProductionProgress(progress) {
  ...
  // calculated_at 每次 API 请求都会变化，但不代表生产状态变化。
  // 把它纳入签名会导致整页每 3 秒重建一次，日志区和手机滚动位置随之跳动。
  const { calculated_at, ...stable } = progress;
  return stable;
}
...
function canvasSig(data) {
  return JSON.stringify([
    ...
    stableProductionProgress(data.production_progress),
