...
def _clear_final_camera_actor_clearance(
    ...
    def clearance(world_point):
        return min(math.hypot(
            float(world_point["x"]) - float(actor["x"]),
            float(world_point["z"]) - float(actor["z"]),
    ...
    def choose(original):
        ...
        nearest = min(actor_world_points, key=lambda actor: math.hypot(
            float(original_world["x"]) - float(actor["x"]),
            float(original_world["z"]) - float(actor["z"])))
        away_x = float(original_world["x"]) - float(nearest["x"])
        away_z = float(original_world["z"]) - float(nearest["z"])
        preferred_angle = (
        ...
        for step in range(1, 41):
            ...
            for angle_step in range(72):
                ...
                angular_change = abs(_wrap_deg(math.degrees(
                    math.atan2(
                        float(world_point["z"]) - float(nearest["z"]),
                        float(world_point["x"]) - float(nearest["x"]))
                    - preferred_angle)))
                candidates.append((
