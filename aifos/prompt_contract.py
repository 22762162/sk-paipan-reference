...
# 镜位/构图裁决条款是平台政策(代码),不是逐镜数据。断点续产会恢复旧
# 冻结合同,审核时必须拿到当前代码的条款全文——12星座 shot 12 的熔断
# 就是恢复了修复前的旧条款。router 审核上下文经此常量新鲜注入。
CAMERA_PRECEDENCE_CLAUSE = (
    ...
    "【空间站位】中的人物-摄影机距离、景深带、画面内/外归属、"
    "屏幕左右标签与遮挡顺序是场景基准机位的投影值;当本镜执行机位"
    "(过肩、反打、环绕等)与取景改变相机几何后,这些投影只供相对"
    "参考:前后层次、遮挡关系、人物是否在画内一律以 camera、构图"
    "合同、population/可见人数合同为准重新投影,屏幕左右以本镜"
    "轴线锚点合同为执行值;空间站位仅保留人物对应、相对距离量级"
    "与场景地理关系,不构成同级互斥,也不需要猜测或裁决。"
    "景别取景边界同为执行值:被边界裁出画的身体"
    "部位、伤情、服装细节或道具,其『必须可见』要求在本镜自动"
    "不适用(由包含该部位的其他镜头承担核验),不构成可见性冲突。"
...
def build_shot_prompt_contract(
    ...
    contract = {
        ...
        "camera_precedence": CAMERA_PRECEDENCE_CLAUSE,
        "physical": physical,
...
def render_shot_prompt(contract, *, mode=None):
    ...
    if composition.get("composition_type") == "over_shoulder_dialogue":
        ...
        lines.append(
            ...
            "本条是本镜唯一执行构图裁决：前景对话者距摄影机最近、以虚化"
            "后脑与肩背局部入画，景别「全景/人物完整入画」只约束画面主体，"
            "不约束过肩前景人物，二者并存不构成互斥。【空间站位】中的"
            "人物-摄影机距离、景深带与遮挡顺序来自场景基准机位投影，与本镜"
            "过肩机位并列时一律以本条前景/主体关系为准重新解释，空间站位"
            "仅保留人物相对位置与场景地理关系，屏幕左右以本镜轴线锚点合同"
            "为准，不构成需要裁决的冲突，也不需要猜测优先级。")
        ...
        lines.append(
            ...
            f"实际可见人形{composition.get('expected_visible_figure_count', visible_count)}人；"
            f"{duties}；{composition.get('count_rule', '')}。"
            "本条是本镜唯一执行构图裁决：近机位肩背局部与该人物全身入画"
            "要求并存时不构成互斥，全身/完整入画要求不适用于近机位局部；"
            "【空间站位】的距离与遮挡数值以本条机位关系为准重新解释，"
            "不构成需要裁决的冲突。")
    if media == "video":
        lines.extend([
