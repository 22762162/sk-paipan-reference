...
- **生产画布**(图片生产实况/图片清单内):图片一开始生产就进入画布
  状态——人物、场景、镜头三列节点由**关系线**牵引(人物×人物同场线、
  人物→场景出场线、场景→镜头所属线、同场镜头帧链),点节点高亮它的
  所有线;同一份关系线会自动注入出图与质检提示词,让后续 AI 始终记住
  人物之间的关联性,多镜之间不漂移;
- **人物设定 · 参考图为最高标准**:每部新剧本开画前,编剧 AI 先为每个
  角色按“剧情证据→经历与处境→性格与行为→可见外貌”推导人物分析与
  视觉 DNA，再从发型、服装结构、身体痕迹、视觉符号、核心配饰和气质
  关键词对全剧角色去重;禁止只靠年龄、性别、帅、美等模板标签;
  角色若有参考图,设定必须以参考图人物的**脸部特征与风格为最高标准**
  撰写(CLI 直接读图,API 以图像块上传);设定生成后再补传参考图,
  会立即按参考图重写该角色设定,再用「批量重画/换风格」按新设定重出;
- **定版后三视图母资产**:人工选中最终人物后，完整版先生成 16:9
  三视图审核板，再分别生成高清面部、正面、严格 90° 侧面和完整 180°
  背面母资产。合成板只用于审核，镜头按视角自动选择对应独立母资产；
  Seedance 只动画化锁定人物与服装，不负责重新设计角色;
...
统一成本记账；默认不设单集金额上限(`budget.per_episode=0`)。如部署方明确
配置非零预算，才启用单集预算熔断。
默认视频生成前必须通过 **14 项门禁**，其中包括剧情事实源、人物母资产、
连续性、空间调度、五维分镜、时长与时间码、台词与语速、表演空间、镜头
语言、人物数量、画面文字、首尾帧、声音设计和生产规格。质检发现可修复的
视频缺失时自动重跑并重新剪辑、复检,最多
...
### 分镜画面:默认每镜 1 张，不满意再按需要备选

母资产之外的**分镜画面不预生成 4 张**——一集几十镜全部四选一意味着四倍
出图量。默认每镜只画 1 张，遇到不满意的镜头由人工按需追加：

```bash
# 对镜头 3 再生成 2 张备选(1-4 张)，可附上哪里不满意
python3 -m aifos alt --project 万妖图录 --episode 1 --shot 3 --count 2 \
    --feedback "主角眼神要更狠"
# 比对后定版第 2 张
python3 -m aifos alt --project 万妖图录 --episode 1 --shot 3 --pick 2
```

- 备选进 `shot_candidate`，**不动本镜当前关键帧**；人工定版后才替换。
- 备选的差异轴只动合同没锁死的维度(表演细节、光影氛围、层次纵深)；
  人物、人数、景别、机位、站位、动作、服装状态和可读文字由五维分镜合同
  锁定，差异轴不得改动。
- 两张以上时由 CODEX 按本镜合同打分**给推荐**，但**不自动换图**——镜头
  定版权始终在人工(母资产才做自动确认)。
- 定版后走与人工上传替换同一条下游路径：同场首尾帧按新图重做、Seedance
  手选参考迁移、旧视频与旧成片作废。
- 网页在分镜面板的「要更多选择?」里选张数并生成；`POST /api/shot/candidates`
  与 `POST /api/shot/select` 是对应接口。
