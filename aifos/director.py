...
# v5:每次图片 API 调用只描述“一张独立单人母图”。“同词抽四张”是
# AIFOS 调度层事实，绝不能写进图片提示词；否则图片模型会把单次输出
# 误解成四宫格候选合集。版本号变化会让 v4 四宫格风险资产自动失效。
CHARACTER_CANDIDATE_PROMPT_SCHEMA = (
    "aifos.character-candidate/v5-single-sample")
...
class Director:
    ...
    def _review_image_tasks(self, ctx, tasks, continue_on_block=False):
        ...
        for group, result in completed:
            ...
            for task in group:
                ...
                reviewed_input = self._image_generation_input(payload)
                payload["_prompt_review_frozen_input_hash"] = \
                    reviewed_input["input_hash"]
                self._plan_mark(
                    ctx, task["item_id"],
    ...
    def _character_sheet_matches_locked_identity(sheet_row, identity_row):
        ...
        另一张脸。候选资产 id 在“同一候选重新锁定”时保持稳定，优先
        用它做依赖键；没有候选 id 的人工身份则退回身份资产 id。
        旧套件没有记录依赖键，按不可信处理并一次性重建。
        """
        if sheet_row is None or identity_row is None:
            return False
        try:
        ...
        if candidate_id is not None:
            return (
                sheet_meta.get("source_candidate_asset_id")
                == candidate_id)
        return (
            sheet_meta.get("source_identity_asset_id")
            == identity_row["id"])
    ...
    def _clean_candidate_initial_state_text(value):
        """剥离淋湿、泥污、伤情、死亡、场次动作姿态等一次性剧情状态。

        磨旧、补丁、旧疤等稳定人物设计仍保留；首次登场即使发生在
        雨中，人物候选母版也只表现自然、干燥、无伤的基础状态。

        姿态尤其关键:定妆母版硬规定「全身正面自然站姿」，若把场次里
        的失去支撑、搀扶、蜷缩等动作姿态写进母版，就与该硬规则直接
        矛盾，提示词审核只能熔断(禁止猜测取舍)，整批人物图卡死。
        """
        text = str(value or "").strip()
        if not text:
            return ""
        ...
        transient = (
            ...
            "眩晕", "骑行", "赶路", "奔逃", "跌倒", "昏迷",
            "松开后", "随行弓兵",
            # 场次动作姿态:与母版硬规则「全身正面自然站姿」互斥,
            # 必须剥离,否则审核判定事实源冲突并熔断整批人物图。
            "失去支撑", "失去平衡", "重心不稳", "站立不稳", "踉跄",
            "瘫软", "倒地", "倒下", "扑倒", "跪地", "跪倒", "半跪",
            "蜷缩", "俯身", "弯腰", "侧卧", "仰卧", "趴伏", "伏地",
            "搀扶", "被扶", "扶住", "撑地", "撑住", "护包", "抱紧",
            "紧抱", "死死抓", "攥紧", "挣扎", "抽搐", "颤抖着",
            "转身", "回头", "奔跑", "疾走", "举手", "抬手", "伸手",
            "指向", "推开", "拉扯", "抬臂格挡", "格挡",
        )
        clauses = []
        for clause in re.split(r"[；;。]", text):
    ...
    def _candidate_prompt_review_context(
        ...
        return {
            "schema": "aifos.character-candidate-review/v5-single-sample",
            "candidate_prompt_schema": CHARACTER_CANDIDATE_PROMPT_SCHEMA,
            "output_scope": (
                "当前图片 API 调用只输出1张独立竖幅单人母图，画面严格"
                "只有1名人物和1个连续完整身体；禁止四宫格、拼图、分栏、"
                "接触表、多视图、候选合集、复制人物或第二人"),
            "orchestration_only": (
                "AIFOS 会在控制层把同一最终提示词复用于4次彼此独立的"
                "图片 API 调用，仅比较随机采样结果。该批次信息只供审核"
                "一致性使用，绝不能写进最终图片提示词；最终图片提示词"
                "不得出现“四张/四图/候选/批次/多视图”等批量措辞"),
            ...
            "source_precedence": (
                "initial_character_state是每次独立单图调用共同且唯一的服装、配色、"
    ...
    def _candidate_portrait_prompt(self, name, role, style, design, variant,
        ...
        props_line = (
            ...
            "造型自动继承手持道具")
        return (
            f"【任务】{name}（{role}）单人初始状态定角母图。"
            "【单次输出硬约束】本次只输出1张独立竖幅图片；画面严格只有"
            "1名人物、1个连续完整身体和1个连续背景。绝对禁止四宫格、"
            "拼图、分栏、接触表、多视图、候选合集、复制人物、分身或第二人。"
            ...
            + "【造型优先级】“共同初始造型”是当前单图唯一的服装、"
            "配色、妆容、配饰与手持道具事实源；不得继承人物全局最终"
            "出图卡、全局服装/标志道具或前后剧情造型。"
            # 事实源里若残留淋湿/泥污/血迹/伤情/动作姿态,与母版硬规则
            # (干净、无伤、正面自然站姿)互斥,审核禁止猜测取舍只能熔断。
            # 明确宣告母版规则优先,把"需要猜"变成"有规可依"。
            + "【母版基准状态·优先级最高】定妆母版只表现人物干燥、洁净、"
            "无伤、无污的基础状态与自然站姿。若上文出现淋湿、水痕、泥点、"
            "泥渍、血迹、伤口、包扎、死亡态,或跪地、蜷缩、搀扶、失去支撑等"
            "场次动作姿态,母版一律不表现；本条优先于上文相应描述，"
            "不需要在两者之间做取舍判断。"
            + f"{variant_rule}。"
    ...
    def _sheet_view_contract(key):
        return {
            ...
            "back": (
                ...
                "扣件与已锁定配饰背部位置；禁止复制正面。"),
            "features": (
                "【FEATURES】正面半身或局部特征板；只展示发型、眼睛、"
                "体态和贴身标志，所有局部都属于同一角色。"),
            "makeup": (
                "【MAKEUP】正面半身中性表情；底妆、眉眼、唇妆和肤质"
                "清晰可见，不加入剧情伤痕或临时污损。"),
            "costume": (
                "【COSTUME】全身正面中性站姿；只展示一套当前主造型，"
                "服装层次、材质与贴身配饰清晰可核对。"),
            "costume_detail": (
                "【COSTUME DETAIL】服装局部与必要的穿着范围；只展示当前"
                "主造型的材质、接缝、扣件和贴身配饰，不展示手持道具。"),
        }.get(key, "")

    @staticmethod
    def _sheet_stable_text(value):
        """Keep only stable, visible character facts for a mother asset.

        Character analysis also contains plot-state facts (mud, blood, wounds,
        travel fatigue) and prop inventories.  Those facts belong to a shot or
        a prop asset, not to every canonical character sheet.  Passing them
        through verbatim makes the image model blend mutually exclusive states.
        """
        ...
        if not text:
            return ""
        # Remove plot-state clauses and parenthetical prop/state notes while
        # retaining the surrounding stable appearance description.
        text = text.replace("面白略带旅途风尘", "面色自然")
        text = re.sub(
        ...
        text = "；".join(
            ...
            if clause.strip() and not re.search(
                r"醒来|左臂|右臂|伤|包扎|血|泥|污", clause))
        text = re.sub(r"[，,；;。]\s*[，,；;。]+", "；", text)
        text = re.sub(r"^[，,；;。]+|[，,；;。]+$", "", text)
        return text.strip()

    @classmethod
    def _sheet_primary_wardrobe(cls, value):
        """Resolve one canonical outfit from a multi-state story description."""
        text = cls._sheet_stable_text(value)
        if not text:
            return ""
        # Prefer the explicitly marked base/current outfit.  If the source has
        # no labels, cut at a known alternate-state marker instead of sending
        # two outfits to the same image request.
        matches = list(re.finditer(
            r"(?:^|[；;|])\s*(本体|基础|默认|当前(?:造型)?|主造型|第一套)"
            r"\s*[:：]\s*", text))
        if matches:
            start = matches[0].end()
            end = matches[1].start() if len(matches) > 1 else len(text)
            alternate = re.search(
                r"[；;|]\s*(?:错穿|备用|第二套|另一套|替换|变装|官服状态)"
                r"\s*[:：]", text[start:])
            if alternate:
                end = min(end, start + alternate.start())
            text = text[start:end]
        else:
            alternate = re.search(
                r"[；;|]\s*(?:错穿|备用|第二套|另一套|替换|变装|官服状态)"
                r"\s*[:：]", text)
            if alternate:
                text = text[:alternate.start()]
            else:
                # Unlabelled detail strings often begin with the base fabric
                # and introduce the alternate uniform after a semicolon.
                alternate = re.search(r"[；;]\s*(?:官服|另一套|备用)", text)
                if alternate:
                    text = text[:alternate.start()]
        return cls._sheet_stable_text(text)

    @classmethod
    def _sheet_attached_accessories(cls, value):
        """Keep wearable identity accessories; never pull handheld props in."""
        text = cls._sheet_stable_text(value)
        if not text:
            return ""
        wearable = (
            "儒巾", "幞头", "头巾", "发簪", "发带", "耳环", "耳坠", "项链",
            "玉佩", "腰牌", "腰带", "素带", "戒指", "手镯", "眼镜", "头饰",
        )
        parts = [part.strip() for part in re.split(r"[；;，,。]", text)
                 if part.strip()]
        kept = [part for part in parts if any(token in part for token in wearable)]
        return "；".join(kept)

    @classmethod
    def _sheet_era_line(cls, value):
        text = cls._sheet_stable_text(value)
        if not text:
            return ""
        # Keep the visible era/world label, not the whole biography or
        # location/action paragraph embedded in some legacy analyses.
    ...
    def _sheet_design_line(cls, design, key, locked_look=None):
        ...
        fields = {
            ...
            "front": ("species", "gender", "age_range", "appearance",
                      "hair", "costume", "costume_detail", "accessories",
                      "palette", "era_setting"),
            "profile": ("species", "gender", "age_range", "appearance",
                        "hair", "costume", "costume_detail", "accessories",
                        "palette", "era_setting"),
            ...
            "costume": ("species", "gender", "age_range", "hair", "costume",
                        "costume_detail", "accessories", "palette",
                        "era_setting"),
            "costume_detail": ("species", "gender", "age_range", "hair",
                               "costume", "costume_detail", "accessories",
                               "palette", "era_setting"),
        }.get(key, ("species", "gender", "age_range", "appearance", "hair"))
        labels = dict(Director.DESIGN_LABELS)
        parts = []
        for field in fields:
            ...
            elif field == "costume_detail":
                value = cls._sheet_primary_wardrobe(value)
            elif field == "accessories":
                value = (locked_look.get("accessories")
                         or design.get("accessories"))
                value = cls._sheet_attached_accessories(value)
            elif field == "era_setting":
        ...
        if isinstance(dna, dict):
            dna_fields = {
                "closeup": ("face_structure", "hair_silhouette",
                            "temperament_keywords"),
                "makeup": ("face_structure", "hair_silhouette"),
                "features": ("face_structure", "hair_silhouette",
                             "body_or_occupation_marks", "signature_accessory"),
                "turnaround": ("face_structure", "hair_silhouette",
                               "clothing_structure"),
                "front": ("face_structure", "hair_silhouette",
                          "clothing_structure"),
                "profile": ("face_structure", "hair_silhouette",
                            "clothing_structure"),
                "back": ("hair_silhouette", "clothing_structure"),
                "costume": ("hair_silhouette", "clothing_structure"),
                "costume_detail": ("clothing_structure",),
            }.get(key, ())
            dna_labels = {
                "face_structure": "脸部骨相",
                "hair_silhouette": "发型轮廓",
                "body_or_occupation_marks": "体态",
                "clothing_structure": "服装结构",
                "signature_accessory": "贴身标志",
                "temperament_keywords": "气质关键词",
            }
            dna_parts = []
            for field in dna_fields:
                if field == "clothing_structure":
                    value = cls._sheet_primary_wardrobe(dna.get(field))
                elif field == "signature_accessory":
                    value = cls._sheet_attached_accessories(dna.get(field))
                else:
                    value = cls._sheet_stable_text(dna.get(field))
                if value:
    ...
    def _sheet_scope_line(key):
        scope = {
            ...
            "features": (
                "只有1人；展示发型、眼睛、体态和贴身标志特征；可做局部放大，"
                "不单独展示手持道具"),
            "makeup": "只有1人、正面半身；只展示底妆、眉眼、唇妆和稳定面部特征",
            "costume": "只有1人、全身正面；只展示一套当前主造型、服装结构和贴身配饰",
            "costume_detail": (
    ...
    def _sheet_prompt(self, name, role, style, label, desc, key=None,
        ...
        key = key or "front"
        detail = self._sheet_design_line(
            design, key, locked_look=locked_look)
        ...
        if detail:
            lines.append(f"【本资产可执行设定】{detail}。")
        if era and key not in ("closeup", "features", "makeup"):
            lines.append(f"【时代】{era}；服装只按本资产的一套主造型执行。")
        if workwear:
            lines.append(f"【职业服装】{workwear}")
        lines.extend([
        ...
        for item in items:
            ...
            if same_content:
                ...
                item["invalidation_reason"] = (
                    ...
                    "旧图片不得按镜头编号继续复用")
                # Codex 就地修镜头合同必然会改变 content_hash。旧图应
                # 作废，但“首图已失败、当前已到第几轮、下一轮具体修复
                # 指令”属于工作流证据，不能跟着旧图一起被清空；否则
                # blocking 阶段先刷新一次清单，images 阶段就会从第0轮
                # 重烧，并丢掉已经写好的第4轮指令。
                if category == "shot_image":
                    prev_qc = prev.get("qc") or {}
                    escalation = prev_qc.get("codex_escalation") or {}
                    preserve_repair_state = bool(
                        prev.get("autonomous_repair_seeded")
                        or prev.get("codex_contract_repair_count")
                        or (
                            prev_qc.get("passed") is False
                            and escalation.get("triggered")
                            and str(escalation.get(
                                "instruction_to_aifos") or "").strip()))
                    if preserve_repair_state:
                        item["status"] = "pending"
                        item["autonomous_repair_seeded"] = True
                        for key in (
                                "codex_contract_repair_count",
                                "codex_contract_repair"):
                            if key in prev:
                                item[key] = copy.deepcopy(prev[key])
                        if (prev_qc.get("passed") is False
                                and escalation.get("triggered")):
                            item["qc"] = copy.deepcopy(prev_qc)
    ...
    def _build_dispatch_contract(self, task, item):
        ...
        if category == "character_sheet":
            # Character sheets are not storyboard shots.  They still need an
            # explicit single-subject contract so the QC worker never falls
            # back to ``expected_visible_figure_count=None`` or interprets a
            # multi-view board as multiple unrelated people.
            sheet_name = str(payload.get("art_name") or "").strip()
            sheet_characters = [str(value) for value in characters if str(value)]
            if sheet_name and sheet_characters != [sheet_name]:
                issues.append(
                    "人物设定图主体合同必须只绑定一个角色: "
                    + ("、".join(sheet_characters) or "未绑定"))
            if payload.get("character_count") != 1:
                issues.append("人物设定图实际可见人形必须明确为1人")
    ...
    def _plan_run(self, ctx, item_id, fn, prompt=None, payload=None,
        ...
        escalation_block = (
            self.escalation_redraw_block(
                payload.get("qc_escalation"), generation_input)
            if self._image_qc_enabled() else "")
        if escalation_block:
            self._plan_mark(
    ...
    def _setting_enabled(value, default=True):
        ...
        if isinstance(value, str):
            ...
            if normalized in ("", "off", "false", "0", "no", "disabled"):
                return False
            if normalized in (
                    "on", "true", "1", "yes", "enabled", "selection"):
                return True
        return bool(value)
    ...
    def _image_qc_enabled(self):
        """图片内容 QC 开关；不影响生成前提示词审核。

        ``image_content_qc`` 是新设置页的权威键；未配置时才
        回退到旧 ``image_qc``，保证历史项目语义不变。选片模式
        只关闭生成后内容判定/返工，Codex 提示词合同仍必审。
        """
    ...
    def _qc_spec(self, project_id, characters, location="", action="",
        ...
        for name in visible_characters:
            ...
            if character_sheet_key:
                line = self._sheet_design_line(
                    ...
                    locked_look=self._locked_look_variant(project_id, name))
            else:
        ...
        return {
            ...
            "static_frame": True,
            "character_sheet_key": character_sheet_key or "",
        }
    ...
    def _resolve_reference_replacement(
        ...
        # A back/profile/closeup sheet requested by Codex is also safely
        # resolvable without an invented URI: only select the latest existing
        # sheet for the named character in the same project.  This is needed
        # for over-shoulder shots where the normal face anchor is insufficient
        # and the requested sheet is already in AIFOS's asset center.
        semantic_role = str(
            selector.get("role") or role or "").strip().lower()
        character = str(selector.get("character") or "").strip()
        identity_detail_roles = {
            "identity_detail", "identity_back_silhouette",
            "identity_profile", "identity_closeup",
        }
        if (semantic_role in identity_detail_roles
                and project_id not in (None, "") and character):
            reason_text = str(reason or "").lower()
            if (semantic_role == "identity_back_silhouette"
                    or any(token in reason_text for token in (
                        "背面", "后脑", "肩背", "back"))):
                sheet_key = "back"
            elif (semantic_role == "identity_profile"
                  or any(token in reason_text for token in (
                      "侧面", "侧脸", "profile"))):
                sheet_key = "profile"
            elif (semantic_role == "identity_closeup"
                  or any(token in reason_text for token in (
                      "正脸", "面部", "五官", "closeup"))):
                sheet_key = "closeup"
            else:
            ...
            if sheet_key:
                row = self.assets.latest(
                    int(project_id), "character_sheet",
                    f"{character}:{sheet_key}")
                uri = str(row["uri"] or "") if row is not None else ""
                if (row is not None and uri
                        and (uri.startswith(("http://", "https://"))
                             or Path(uri).exists())
                        and not self.assets.is_deleted(row)):
                    return row
    ...
    def _apply_image_reference_adjustments(
        ...
        for action in actions:
            ...
            if kind == "keep":
                continue
            # For "add", target_index is often the related identity anchor,
            # not an asset to mutate. Treating it as the target makes the
            # protected-identity guard reject a harmless same-character back
            # or profile supplement before semantic resolution can run.
            target = (
                None if kind == "add"
                else manifest.get(action.get("target_index")))
            target_role = str(
                action.get("role")
                if kind == "add" and action.get("role")
                else (target or {}).get("role") or "")
            ...
            if kind in ("replace", "add"):
                ...
                role = (
                    ...
                    if requested_role in {
                        "identity_back_silhouette",
                        "identity_profile", "identity_closeup",
                    }
                    else requested_role)
                character = str(
                    action.get("character")
                    or (action.get("replacement_selector") or {}).get(
                        "character") or "")
                if kind == "replace":
    ...
    def _generate_selection_candidates_parallel(
        ...
        frozen_input = self._image_generation_input(
            working_payload, qc_spec=qc_spec)
        # 候选 worker 只能复用这一次已批准的冻结合同。marker 必须与
        ...
        version = build_candidate_set_version(
            episode_id=working_payload.get("_episode_id"),
            shot_no=working_payload.get("shot_no"),
            contract_revision=working_payload.get(
                "_contract_revision") or 1,
            candidate_revision=working_payload.get(
                "_candidate_revision") or 1,
            prompt=frozen_input["prompt"],
            reference_manifest=frozen_input["reference_manifest"],
        )
        ...
        def generate_one(index):
            ...
            seed = int(hashlib.sha256(
                f"{candidate_set_id}:{index}".encode("utf-8")
            ).hexdigest()[:8], 16)
            candidate_payload.update({
                "_candidate_set_id": candidate_set_id,
                "_candidate_index": index,
                "_candidate_seed": seed,
                "seed": seed,
                "candidate_set_token": result_version.candidate_set_token,
                "candidate_index": result_version.candidate_index,
        ...
        if (isinstance(resume_group, dict)
            ...
            for candidate in resume_group.get("candidates") or []:
                ...
                reusable = bool(
                    1 <= index <= pulls
                    and candidate.get("candidate_set_token") == version.token
                    and candidate.get("prompt_hash") == frozen_prompt_hash
                    and candidate.get("reference_hash")
        ...
        def run_wave(indices, attempt):
            ...
            with ThreadPoolExecutor(
                ...
                futures = {
                    pool.submit(generate_one, index): index
                    for index in indices}
                for future in as_completed(futures):
                    index = futures[future]
                    try:
                        got_index, result, candidate = future.result()
                        if (candidate.get("prompt_hash") != frozen_prompt_hash
                                or candidate.get("reference_hash")
                                != frozen_reference_hash
                                or candidate.get("input_hash")
                                != frozen_input_hash):
                            raise AifosError(
                                f"候选图{index}偏离冻结提示词/参考图合同")
                        candidate["technical_attempts"] = attempt
                        results[got_index] = (result, candidate)
        ...
        same_prompt = all(
            row["prompt_hash"] == frozen_prompt_hash for row in candidates)
        same_references = all(
        ...
        # result.uri 偷偷让第一张流入首尾帧/视频参考链。
        selected.uri = ""
        if not isinstance(selected.data, dict):
            selected.data = {}
        ...
        if isinstance(report, dict):
            report["gacha"] = {
                ...
                "recommended_candidate_index": recommended_index,
                "recommended_score": float(
                    recommended_candidate.get("score") or 0),
                "candidate_set_id": candidate_set_id,
                "frozen_prompt_hash": frozen_prompt_hash,
                "frozen_reference_hash": frozen_reference_hash,
                "same_prompt": same_prompt,
    ...
    def _generate_image_gacha(self, capability, payload, out_dir,
        ...
        selection_group = bool(
            (payload or {}).get("_selection_candidate_group"))
        ...
        for pull in range(1, pulls + 1):
            pull_payload = copy.deepcopy(working_payload)
            result = self._generate_image_with_qc(
                capability, pull_payload, out_dir, cancel,
                qc_spec)
            report = getattr(result, "qc", None) or {}
            ...
            optimized_prompt = str(
                result_data.get("prompt_optimized") or "").strip()
            prompt_review = result_data.get("prompt_review")
            if select_after_all and pull == 1 and optimized_prompt:
                working_payload["prompt"] = optimized_prompt
                if "prompt_compact" in working_payload:
                    working_payload["prompt_compact"] = optimized_prompt
                if isinstance(prompt_review, dict):
                    working_payload["prompt_review"] = copy.deepcopy(
                        prompt_review)
                if working_payload.get("feedback"):
                    working_payload["prompt_review_feedback_applied"] = \
                        working_payload["feedback"]
                    working_payload["feedback"] = ""
                frozen_prompt_hash = hashlib.sha256(
                    optimized_prompt.encode("utf-8")).hexdigest()
            candidate_prompt = (
                optimized_prompt
                or str(pull_payload.get("prompt_compact")
                       or pull_payload.get("prompt") or ""))
        ...
        if isinstance(final_report, dict):
            final_report["gacha"] = {
                ...
                "same_prompt": bool(
                    frozen_prompt_hash
                    and all(row.get("prompt_hash") == frozen_prompt_hash
                            for row in candidates)),
                "candidates": copy.deepcopy(candidates),
            }
        if final_rank[0]:
    ...
    def _repair_gacha_prompt_invariant_valid(plan_item):
        ...
        qc = (
            ...
            if isinstance(plan_item, dict) else {})
        gacha = qc.get("gacha") or {}
        if not isinstance(gacha, dict) or not gacha.get("select_after_all"):
            return True
        candidates = gacha.get("candidates") or []
        ...
        return bool(
            ...
            and all(
                isinstance(row, dict)
                and str(row.get("prompt_hash") or "") == frozen
                for row in candidates))
    ...
    def _shot_candidate_group_valid(plan_item):
        ...
        if (group.get("schema") != "aifos.shot-candidate-group/v1"
                ...
                or str(version.get("token") or "") != token
                or group.get("same_prompt") is not True
                or group.get("same_references") is not True):
            return False
        for row in candidates:
            if str((row or {}).get("candidate_set_token") or "") != token:
                return False
            uri = str((row or {}).get("uri") or "")
            if not uri:
                return False
            if (not uri.startswith(("http://", "https://"))
                    and not Path(uri).exists()):
    ...
    @staticmethod
    def _candidate_diagnostic_uri(result):
        """严格QC模式用于诊断的候选，不代表已晋升正式资产。"""
        group = ((getattr(result, "data", None) or {}).get(
            "candidate_group") or {})
        candidates = group.get("candidates") or []
        try:
            index = int(group.get("recommended_candidate_index") or 1)
        except (TypeError, ValueError):
            index = 1
    ...
    def _generate_image_with_qc(self, capability, payload, out_dir,
        ...
        while True:
            ...
            current_input = self._image_generation_input(
                payload, qc_spec=qc_spec)
            frozen_review_hash = str(
                payload.get("_prompt_review_frozen_input_hash") or "")
            if not (frozen_review_hash
                    and frozen_review_hash == current_input["input_hash"]):
                reviewer = getattr(
                    self.router, "review_image_prompt", None)
                prompt_review = (
                    reviewer(capability, payload, out_dir, cancel=cancel)
                    if callable(reviewer) else None)
    ...
    @staticmethod
    def _candidate_group_technical_incomplete(result):
        data = getattr(result, "data", None) or {}
    ...
    def _candidate_selection_pending(result):
        data = getattr(result, "data", None) or {}
        group = data.get("candidate_group") or {}
        if not isinstance(group, dict) or not group.get("selection_required"):
            return False
        selection = group.get("selection") or {}
        if not isinstance(selection, dict):
            return True
        selected_uri = str(selection.get("selected_uri") or "").strip()
        selected = next((
            row for row in group.get("candidates") or []
            if str((row or {}).get("uri") or "") == selected_uri
        ), None)
        if selected is None:
            return True
        try:
            ...
            selected_result = CandidateResultVersion(
                candidate_set_token=str(
                    selection.get("candidate_set_token")
                    or selection.get("token") or ""),
                candidate_index=int(
                    selection.get("candidate_index")
                    or selected.get("candidate_index") or 0),
            )
        except (TypeError, ValueError):
            return True
        # URI、序号、版本令牌三者必须指向同一张当前候选；旧页面的
        # 迟到点击即使带着合法 manual/ai 来源也不能覆盖新候选组。
        if (selected_result.candidate_index
                != int(selected.get("candidate_index") or 0)):
            return True
        decision = evaluate_candidate_promotion(
            selected_result, current_version,
            selection_source=str(selection.get("source") or ""))
        return not decision.allowed

    def _critical_qc_error(self, result):
    ...
    # 同一镜头最多自动改几次合同。前两轮落实逐项诊断；第三轮起不再
    # 继续叠加提示词，而是每轮都让编剧按最新诊断重建一份“保剧情、
    # 删过约束”的短合同。三轮后 Codex 仍可能给出新的、可执行的修复
    # （例如改角色构图职责或补背面参考），不能把这类已明确的第4轮
    ...
    DEEP_CONTRACT_SLIMMING_GUIDANCE = (
        ...
        "铁律:运镜(推/拉/摇/移/跟/升降/环绕及表现性运镜)是视频"
        "专属维度,与静态关键帧的可拍性无关——瘦身禁止删除、禁改"
    ...
    @staticmethod
    def _prompt_review_block_reason(exc):
        """Return a content-review block, excluding provider/infrastructure errors."""
        if not isinstance(exc, ProviderError) or isinstance(
                exc, ProviderUnavailable):
            return ""
        reason = str(exc)
        if ("真实图片已被阻止" not in reason
                and "拒绝生图" not in reason):
            return ""
        return reason

    def _auto_repair_prompt_review_block(self, ctx, task, exc):
        """Repair an in-flight shot when its newly revised prompt is blocked.

        Prompt review runs again inside every real generation call.  Previously,
        a shot could pass the batch pre-review, fail visual QC, receive and apply
        ...
        item_id = str(task.get("item_id") or "")
        if not reason or not item_id.startswith("shot:"):
            return ""
        counters = ctx.setdefault("_codex_contract_repairs", {})
        payload = task.get("payload") or {}
        try:
        ...
        if used >= self.CODEX_CONTRACT_REPAIR_LIMIT:
            return ""
        repair_reason = (
            "生成前提示词审核发现最新镜头合同仍有冲突。"
            "不得把本镜标成等待人工；请就地输出唯一可拍合同并删除旧冲突句。"
            f"\n审核阻断原因:{reason[:2400]}")
        if used >= 2:
            repair_reason += "\n" + self.DEEP_CONTRACT_SLIMMING_GUIDANCE
        try:
            ...
            self.log.warn(
                "director",
                f"{item_id} 生成前审核冲突自动修复失败:"
                f"{str(repair_exc)[:300]}")
            return ""
        payload = task.get("payload") or {}
        next_round = used + 1
        contract = copy.deepcopy(payload.get("shot_contract") or {})
        resolution = {
            "revision_round": next_round,
            "policy": (
                "本轮 camera、shot_contract、action 是最新镜头局部合同；"
                "它们替代并作废旧提示词、旧模型约束和旧审核稿中与之冲突"
                "的景别、焦段、机位、构图、运镜、视线、裁切和道具终态。"),
        ...
        payload.pop("qc_escalation", None)
        payload["feedback"] = (
            f"【第{next_round}轮生成前审核冲突裁决·最高优先级】"
            + resolution["policy"]
            + f"唯一镜位={payload.get('camera') or contract}；"
            f"唯一静态终态={str(payload.get('action') or '')[:900]}")
        task["payload"] = payload
        if task.get("qc_spec"):
            task["qc_spec"] = self._shot_qc_spec(ctx, payload)
        self._attach_reference_manifest(payload)
        counters[item_id] = next_round
        self.log.info(
            "director",
    ...
    def _run_one_task(self, ctx, task, *, continue_on_qc_failure=False):
        ...
        if callable(callback):
            try:
                ...
                # render_plan=done 已经是可恢复的权威断点；即时资产登记若
                # 失败，下一轮 reconcile_completed_shot_images 仍会补登。
                # 不能因为资产中心的一次短暂写故障把已通过图片重新判失败。
                self.log.warn(
                    "director",
                    f"{task.get('item_id')} 通过后即时登记资产失败，"
                    f"断点对账将自动补登: {str(exc)[:180]}")
        return result
    ...
    def prop_selection_status(self, project_id, script):
        """核心道具定版状态：每件四选一；普通一次性小物不进入此门禁。"""
        result = []
        candidate_rows = {}
        for row in self.assets.list(project_id, "prop_candidate"):
            candidate_rows[row["name"]] = row
        for prop in core_prop_definitions(script):
            name = prop["name"]
            ...
            for row in candidate_rows.values():
                ...
                if index < 1 or index > PROP_CANDIDATES:
                    continue
                candidates.append({
                    "id": f"prop_candidate:{name}:{index}",
                    "index": index,
                    "uri": uri,
                    "version": row["version"],
                    "variant_id": meta.get("variant_id", ""),
            ...
            result.append({
                ...
                "candidate_target": PROP_CANDIDATES,
                "candidate_count": len(candidates),
                "locked": locked is not None,
                "identity_uri": locked["uri"] if locked else "",
                "identity_version": locked["version"] if locked else None,
        ...
        return {
            "schema": "aifos.prop-selection/v1",
            "candidate_target": PROP_CANDIDATES if result else 0,
    ...
    def production_asset_selection_status(self, project_id, script):
    ...
    def _prop_candidate_variant(index):
        variants = (
            ("baseline", "基准造型", "最清晰地落实剧本功能、时代与基本轮廓"),
            ("silhouette", "轮廓结构", "优化外形比例、握持结构与远景辨识度"),
            ("material", "材质工艺", "优化材质、工艺、磨损和年代使用痕迹"),
            ("story_detail", "剧情细节", "强化有明确来源的功能细节与故事识别点"),
        )
        variant_id, label, focus = variants[index - 1]
        return {
    ...
    def _prop_candidate_review_context(self, prop, style, variant):
        ...
        return {
            ...
            "output_scope": (
                "当前图片 API 调用只输出1张独立单件道具母图；禁止"
                "四宫格、拼图、分栏、接触表、多视图、候选合集、"
                "复制道具或第二件道具"),
            "prop_facts": {
            ...
            "master_state_precedence": (
                "本合同是道具母版:只表现道具本体的基准形态。名称或"
                "visual_design明确规定的永久固有识别结构（如固定缺角、"
                "断纹、旧磕痕、长期磨损、残留旧丝）必须保留；任何来源"
                "中的外包装(油纸、布套、匣盒)、场次新产生的污渍、泥点、"
                "血迹、水痕、临时破损或临时折裂,母版一律不表现。"
                "本条优先级最高,与其他事实并列出现时直接按本条执行,"
                "不构成需要裁决的冲突,也不需要猜测优先级"),
            "text_policy": (
    ...
    def _prop_candidate_prompt(self, prop, style, variant):
        ...
        text_rule = (
            ...
            "无新增文字、无字幕、无Logo、无水印；")
        single_face_rule = (
            "【单面视图】本张只展示正面；视觉结构中明确属于背面的纹样、"
            "刻记或附件不得挪到正面，背面内容在本视角不可见。"
            if "正面" in facts and "背面" in facts else "")
        return (
            f"【任务】核心道具「{prop['name']}」独立单件母图 · "
            f"{variant['variant_label']}。"
            "【单次输出硬约束】本次只输出1张独立图片，画面严格只有"
            "1件道具和1个连续背景；绝对禁止四宫格、拼图、分栏、"
            "接触表、多视图、候选合集、复制道具或第二件道具。"
            f"【道具事实】{facts or '外形、功能、材质与时代必须服从正式剧本'}。"
            # 事实源常把外包装、污损、破口写进视觉结构。母版必须给出
            # 确定的取舍规则,否则审核面对"包装 vs 无包装"只能判冲突
            # 并熔断(规则禁止猜测)。这里明确宣告优先级,消除歧义。
            "【母版基准状态·优先级最高】只画道具本体的基准形态。名称或"
            "视觉结构明确规定的永久固有识别结构（如固定缺角、断纹、"
            "旧磕痕、长期磨损、残留旧丝）必须保留；若上文事实中出现"
            "外包装(油纸、布套、匣盒等)、场次新产生的污渍、泥点、"
            "血迹、水痕、临时破损、临时折裂或其他临时状态,母版一律"
            "不画；本条优先于事实源中的相应描述,不需要在两者之间做"
            "取舍判断。"
            f"{text_rule}"
            f"{single_face_rule}"
    ...
    def _ensure_prop_candidates(self, ctx, props, style):
        ...
        for prop in props:
            ...
            quality = resolve_image_quality(
                recommend_asset_quality(
                    "prop_asset", reuse_count=2, core_prop=True),
                ctx.get("quality_policy") or default_quality_policy(),
                f"prop_candidate:{name}")
            for index in range(1, PROP_CANDIDATES + 1):
                if locked and index not in existing:
                    continue
                variant = self._prop_candidate_variant(index)
                prompt = self._prop_candidate_prompt(prop, style, variant)
                item_id = f"prop_candidate:{name}:{index}"
                quality_by_candidate[(name, index)] = quality
                seed.append({
                    "id": item_id,
                    "category": "prop_candidate",
                    "label": (
                        f"{name} · 候选 {index} · "
                        f"{variant['variant_label']}"),
                    "name": name,
                ...
                tasks.append({
                    ...
                    "sub_dir": "cast/props/candidates",
                    "tag": (name, index),
                })
        self._plan_seed(ctx, "prop_candidate", seed)
        current = self.prop_selection_status(
            project_id, {"core_props": props})
        for item in seed:
            if any(
                    candidate["index"] == item["candidate_index"]
                    for prop in current["props"]
                    if prop["prop"] == item["name"]
    ...
    def select_prop_candidate(self, project_title, episode_number,
        ...
        if index < 1 or index > PROP_CANDIDATES:
            raise AifosError(
                f"{prop_name}固定为{PROP_CANDIDATES}张候选，"
                f"不能选择第{index}张")
        candidate = self.assets.latest(
            project["id"], "prop_candidate",
            f"{prop_name}:{index:02d}")
        if candidate is None or not candidate["uri"]:
            raise AifosError(f"道具候选不存在: {prop_name}/{index}")
        uri = str(candidate["uri"])
        if not uri.startswith(("http://", "https://")) and not Path(uri).exists():
            raise AifosError("候选图片文件已丢失，请重新生成候选")
        quality = self._asset_quality(candidate, default="high")
        if not formal_reference_allowed(quality):
            raise AifosError("低质量试错图不能锁为正式道具参考")
        candidate_meta = self._asset_meta(candidate)
        identity = self.assets.register(
            project["id"], "prop_identity", prop_name,
            uri=uri,
            meta={
                "prop": prop_name,
                "locked": True,
        ...
        ctx = {"episode": dict(episode),
               "out_root": self._episode_dir(project, episode)}
        for item in self._plan_read(ctx).get("items", []):
            if (item.get("category") == "prop_candidate"
                    and item.get("name") == prop_name):
                self._plan_mark(
                    ctx, item["id"], item.get("status", "done"),
                    extra={
                        "selected": int(
                            item.get("candidate_index", 0)) == index})
        self._plan_mark(
    ...
    def _stage_cast(self, ctx):
        ...
        for character in characters:
            name = character["name"]
            role = character.get("role", "")
            locked_identity = self._locked_identity(project_id, name)
            locked_identity_meta = json.loads(
                locked_identity["meta"] or "{}")
            ...
            for key, label, desc in sheet_definitions:
                ...
                if existing_sheet:
                    ...
                    if (self._quality_meets(
                            ...
                            and self._character_sheet_matches_locked_identity(
                                row, locked_identity)):
                        reused += 1
                        self._plan_mark(
                        ...
                        continue
                    self._plan_mark(
                        ctx, f"sheet:{name}:{key}", "pending",
                        extra={
                            "invalidated_previous_output": True,
                            "invalidation_reason": (
                                "最终立绘候选已变化或旧套件缺少身份依赖键，"
                                "必须从当前锁定身份重建"),
                            "source_candidate_asset_id": (
                                locked_identity_meta.get(
                                    "candidate_asset_id")),
                        })
                tasks.append({
                    ...
                    "payload": self._with_reference_manifest({
                        ...
                        "sheet_prompt_schema": "aifos.character-sheet/v2",
        ...
        for tag, result in self._run_parallel(
            ...
            self.assets.register(
                ...
                meta={"character": name, "sheet": key, "label": label,
                      ...
                      "source_identity_asset_id": locked_identity["id"],
                      ...
                      "fresh_run_id": (
                          ctx.get("run_id")
                          if ctx.get("fresh_assets") else None)},
                new_version=bool(self.assets.latest(
                    project_id, "character_sheet", f"{name}:{key}",
                    include_deleted=True) is not None))
            created += 1
    ...
    def _scene_layout_clause(self, ctx, location, spatial):
        ...
        三维场景表由 build_scene_model 从 720° 全景反解(物体位置是几何
        解出来的,不是估计)。它把家具钉死在坐标上,是「物品位置固定、
        镜头不穿帮」的事实源。
        """
        location = str(location or "").strip()
        if not location:
        ...
        try:
            ...
            return ""
        return scene_layout_clause(model, (spatial or {}).get("camera"))
    ...
    def _shot_payload(self, ctx, shot, *, continuity_anchor=False,
        ...
        payload = {
            ...
            **prop_contract,
            ...
            "scene_layout": self._scene_layout_clause(ctx, location, spatial),
            "era_context": era_context,
            "sanctioned_anachronisms": sanctioned_anachronisms,
    ...
    def _reference_manifest(self, payload):
        ...
        add(
            ...
            "比例严格保持约1.8头身，头占总高约58%，身体和四肢明显小于头；"
    ...
    def reconcile_completed_shot_images(self, ctx):
        ...
        plan = self._plan_read(ctx)
        by_id = {item.get("id"): item for item in plan.get("items", [])}
        image_content_qc = self._image_qc_enabled()
        recovered = 0
        exposed_failures = 0
        changed = False
        ...
        for shot in (ctx.get("storyboard") or {}).get("shots", []):
            ...
            if not image_content_qc and not candidate_group_valid:
                ...
                continue
            if (qc.get("passed") is True
                    and not self._repair_gacha_prompt_invariant_valid(item)):
                item["status"] = "failed"
                qc["passed"] = False
                qc["stale"] = True
                qc["autonomous_repair_required"] = True
                qc["retry_blocked"] = False
                qc["issues"] = list(dict.fromkeys([
                    *(qc.get("issues") or []),
                    "修订组候选未证明使用同一冻结提示词，"
                    "该批作废并由AIFOS自动重抽",
                ]))
    ...
    def _video_reference_mismatch(self, row, shot, location="", script=None):
        ...
        if kind == "prop_candidate":
            return "未定版的道具候选禁止作为 Seedance 正式参考"
        if kind == "prop_identity":
            prop = str(
    ...
    def _build_technical_video_qc_report(
            self, ctx, qc_report, previous=None):
        """关闭视频内容QC后只保留产物/媒体健全性，永不待人工。"""
        previous = previous or {}
        previous_shots = {
            str(item.get("shot_no")): item
            for item in previous.get("shots", [])
        ...
        for shot in (ctx.get("storyboard") or {}).get("shots", []):
            ...
            related = [
                ...
                if issue.get("check") in shot_checks
                and issue.get("shot_no") == shot_no
            ...
            messages = list(dict.fromkeys(
                ...
                for issue in related))
            old = previous_shots.get(str(shot_no), {})
            used = int(old.get("auto_retries_used") or 0)
            retry_available = bool(
                messages
                and any(issue.get("rerunnable") for issue in related)
                and used < VIDEO_QC_AUTO_RETRIES)
            decision = {
                "action": (
                    "direct_video_retry" if retry_available else "none"),
                "safe_to_auto_retry": retry_available,
                "input_changed": retry_available,
                "prompt_patch": (
                    "技术恢复：严格沿用已冻结的本镜首尾帧、"
                    "提示词和参考图，重新生成并完整落盘"
            ...
            shots.append({
                ...
                "generation_attempts": int(
                    old.get("generation_attempts") or 1),
                "auto_retries_used": used,
                "auto_retry_limit": VIDEO_QC_AUTO_RETRIES,
                "revision_feedback": "",
                "revision_categories": ["technical"] if messages else [],
                "decision": decision,
                "awaiting_human": False,
                "content_qc_enabled": False,
            })
        failed = [item for item in shots if not item["passed"]]
        global_issues = list(dict.fromkeys(
            str(issue.get("message") or issue.get("check"))
            for issue in issues
            if issue.get("check") in global_checks
            and issue.get("shot_no") is None
            and issue.get("severity") == "error"))
        content_observations = [
            copy.deepcopy(issue) for issue in issues
            if issue.get("check") not in global_checks
            and issue.get("check") not in shot_checks]
        return {
            "schema": VIDEO_QC_SCHEMA,
            "passed": not failed and not global_issues,
            "shots": shots,
            ...
            "global_status": "failed" if global_issues else "passed",
            "auto_retry_limit": VIDEO_QC_AUTO_RETRIES,
            "last_total_qc_passed": bool(qc_report.get("passed")),
            "content_qc_enabled": False,
    ...
    def _stage_qc(self, ctx):
        ...
        if not video_content_qc:
            content_review["advisory_only"] = True
            content_review["blocking"] = False
            content_review["waived_by"] = (
                "selection_mode_or_video_content_qc")
        ...
        for attempt in range(max_rounds):
            ...
            video_qc = (
                ...
                if video_content_qc else
                self._build_technical_video_qc_report(
                    ctx, report, previous=video_qc))
            ...
            if video_shots:
                ...
                video_qc["auto_retry_count"] = sum(
                    ...
                    for item in video_qc.get("shots", []))
                self._save_video_qc_report(ctx, video_qc)
        delivery_review = content_review
        if not video_content_qc:
            # 交付脚本仍校验文件、数量、成片和标准指纹；
            # 只把已关闭的内容判定从「passed」计算中豁免。
            delivery_review = {**content_review, "passed": True}
        delivery = write_delivery_verifier(
            ctx, review_board, delivery_review)
        ...
        if not video_content_qc:
            ...
            for issue in report["issues"]:
                ...
                if item.get("check") not in technical_checks:
            ...
            technical_errors = [
                ...
                if issue.get("check") in technical_checks
                and issue.get("severity") == "error"]
            report["technical_passed"] = not technical_errors
            report["content_observations"] = observations
            report["content_observed_passed"] = bool(
                content_review.get("passed"))
            report["content_passed"] = True
            report["content_qc_enabled"] = False
            report["content_qc_waived"] = True
            report["score"] = max(0, 100 - 15 * len(technical_errors))
            report["passed"] = bool(
                report["technical_passed"] and delivery.get("passed"))
            report["rerun_shots"] = sorted({
                int(issue["shot_no"]) for issue in technical_errors
                if issue.get("shot_no") is not None
                and issue.get("rerunnable")})
            report["rerun_lines"] = sorted({
                int(issue["line_no"]) for issue in technical_errors
        ...
        video_qc["awaiting_human"] = (
            bool(video_qc.get("awaiting_human_shots")
                 or video_qc.get("global_issues"))
            if video_content_qc else False)
        self._save_video_qc_report(ctx, video_qc)
    ...
    def _invalidate_cast_assets(self, project, script, reason,
        ...
        for character in characters:
            ...
            # 第5张是旧版5/3/1规则的历史候选；也写入新墓碑，
            # 防止资产中心继续把它误显示为当前可选图。
            for index in range(
                    1, LEGACY_CHARACTER_CANDIDATE_MAX + 1):
                self.assets.register(
                    project["id"], "character_candidate",
        ...
        for prop in props:
            name = prop["name"]
            self.assets.register(
                project["id"], "prop_identity", name, uri="",
                meta={"prop": name, "locked": False, "reason": reason},
                new_version=True)
            for index in range(1, PROP_CANDIDATES + 1):
                self.assets.register(
                    project["id"], "prop_candidate",
    ...
    def direct_shot(self, project_title, episode_number, shot_no, *,
        ...
        编译(redo_items 既有行为),生成/审核/质检全链按新指令执行。
        ...
        picked = {k: str(v).strip() for k, v in camera.items()
                  if str(v or "").strip()}
        if picked:
            allowed = {"景别", "角度", "机位", "运镜", "构图"}
            illegal = sorted(set(picked) - allowed)
            if illegal:
                raise AifosError("未知镜头维度: " + "、".join(illegal))
            original = shot.get("camera")
            if isinstance(original, dict):
                original.update(picked)
            else:
                # 字符串镜头:按维度顺序重组,未指定的维度保留原词
                text = str(original or "")
                tokens = [t for t in text.split("·") if t]
                by_dim = {}
                from .camera_language import (ANGLE_GEOMETRY,
                ...
                for token in tokens:
                    for dim, table in tables.items():
                        if token in table and dim not in by_dim:
                            by_dim[dim] = token
                            break
                by_dim.update(picked)
                ordered = [by_dim.get(d) for d in
                           ("景别", "角度", "机位", "运镜", "构图")]
                extras = [t for t in tokens
                          if all(t not in table for table in tables.values())]
                shot["camera"] = "·".join(
                    [v for v in ordered if v] + extras)
            changed.append("镜头:" + "、".join(
                f"{k}={v}" for k, v in picked.items()))
        if str(lighting_style or "").strip():
            from .lighting_language import LIGHTING_STYLES
            key = str(lighting_style).strip()
            if key == "auto":
                shot.pop("lighting_style", None)
                changed.append("光影:恢复自动选型")
            elif key in LIGHTING_STYLES:
                shot["lighting_style"] = key
                changed.append(
                    "光影:" + LIGHTING_STYLES[key]["label"])
            else:
                raise AifosError(f"未知光影风格: {key}")
        if clear_note:
            shot.pop("director_note", None)
            changed.append("已清除导演意见")
    ...
    def restyle_project(self, project_title, episode_number, style=None):
        ...
        self._invalidate_cast_assets(
            project, script, reason="restyle",
            invalidate_scenes=True)
    ...
    def qc_all(self, project_title, episode_number, *,
        ...
        if not self._image_qc_enabled():
            return {
                "status": "done", "checked": 0, "passed": 0,
                "failed": 0, "auto_repaired": 0,
                "skipped": True,
                "reason": "选片模式或图片内容质检已关闭",
            }
        project, episode = self._episode_ctx(project_title, episode_number)
        ctx = {"project": dict(project), "episode": dict(episode),
               "out_root": self._episode_dir(project, episode)}
    ...
    def expand_scene_views(self, project_title, location, *,
        ...
        # ② 四向视角:真实 2:1 全景优先走 ffmpeg v360 数学投影。
        # 让图片模型“参考全景再画一个 180° 反打”会频繁返回近似正向
        # 构图，画面像同一个房间但机位并没有真的旋转。只有 mock/旧式
        # 非全景输入才保留 AI 方向图兼容路径。
        ...
        for key, label, degrees, desc in self.SCENE_DIRECTION_DEFS:
            ...
            if deterministic_slices:
                safe_location = "".join(
                    c if c.isalnum() else "_" for c in location)[:60]
                slice_dir = (
                    out_dir / ".panorama_slices"
                    / f"{safe_location}_v{int(panorama_row['version'])}")
                yaw = degrees if degrees <= 180 else degrees - 360
                slice_uri = slice_panorama(
                    base_uri, slice_dir, yaw=yaw, pitch=0, h_fov=90,
                    size=(ASPECT_DIMS["16:9"]["width"],
                          ASPECT_DIMS["16:9"]["height"]))
                if slice_uri:
                    result = SimpleNamespace(
                        provider="panorama_slice",
                        model="ffmpeg-v360",
                        cost=0.0,
                        data={
                            "source_panorama": base_uri,
            ...
            saved, uri = register(
                ...
                extra={"degrees": degrees,
                       ...
                       "deterministic_projection": bool(
                           result.provider == "panorama_slice"),
                       "panorama_asset_id": (
