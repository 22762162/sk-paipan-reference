...
ISSUE_TAXONOMY = (
    # 判官措辞多变:"人数少一人""画面只有6人""少了1名弓兵""缺少第4名"
    # 都是同一类。分类器过度字面化会把它们全丢进 other,台账失真、
    # 跨轮比对也失效(实测 other 是最大桶)。
    ("figure_count", re.compile(
        r"人数|多余人物|新增人物|复制人物|检测到\S{0,4}人"
        r"|只有\s*[\d一两三四五六七八九十]+\s*[人名位]"
        r"|少[了于]?\s*[\d一两三四五六七八九十]*\s*[人名位]"
        r"|缺少[^。；]{0,40}?第?\s*[\d一两三四五六七八九十]+\s*[人名位]"
        r"|多[了出]\s*[\d一两三四五六七八九十]*\s*[人名位]")),
    ("gender", re.compile(r"性别|画成\S{0,4}(?:少女|女性|男性|少年)"
                          r"|女性表达|男性表达")),
    ("species", re.compile(r"物种|画成了?\S{0,4}(?:动物|野兽|机器|物体)"
                           r"|兽形|非人形")),
    ("identity_drift", re.compile(
        r"身份|立绘不一致|骨相|五官|脸型|年龄感|漂移|面部\S{0,6}不一致"
        r"|画成\S{0,6}(?:另一|别的)人|与参考图\S{0,6}不符")),
    ("camera", re.compile(
